...
- **The FSRS interval runs from the last sighting of a card, not the last scoring review.** A phase-3 re-read moves the decay clock without moving the difficulty and stability it decays, so the elapsed time and the FSRS values now come from different rows. The old value overstated elapsed time by up to one session's length, scheduling slightly tighter than intended.

...
NOTES: Instead of an RPC (which would bypass RLS and go against local-first principles), implemented a BEFORE INSERT trigger on `friend_request_action` that:
- Validates state transitions (can't accept your own invite, can't remove if not friends, etc.)
- Handles mutual invite edge case: if A invites B, then B invites A, the second invite is auto-transformed to 'accept' (both parties consented)
- Keeps the direct insert pattern intact (client still inserts to table)
- RLS handles auth (who can insert), trigger handles business logic (what actions are valid)

Test file: `supabase/test-friend-request-trigger.sql` - 13 test scenarios all passing.

This approach treats consent as seriously as security: no one can become friends without mutual consent.

...
1. **Created new feed hooks** (`src/hooks/use-feed.ts`):
   ...
   - `usePopularFeedLang(lang)`: Queries `feed_activities` ordered by `payload->upvote_count` descending, showing most upvoted content first.

2. **Updated feed components** (`src/routes/_user/learn/$lang.feed.tsx`):
...
3. **Removed old hooks**: No longer using `useMyFriendsRequestsLang` and `usePopularRequestsLang` from `use-requests.tsx` in the feed page (these only returned requests, not polymorphic items).

All 6 feed tests pass.

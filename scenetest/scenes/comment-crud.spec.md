...
// friend opens a partial-lang request, posts a comment, then uses the
...
- click phrase-picker-item [team.partial_attach_phrase]
...
// [team.partial_phrase_linked_seed_comment] also has a phrase link and is seeded with a
...
// Cleanup preserves [team.partial_learner_seed_comment] so it remains
// available as a reply target for the next run.

cleanup: supabase.from('request_comment').delete().eq('uid', '[learner.key]').eq('request_id', '[team.partial_crud_request]').neq('id', '[team.partial_learner_seed_comment]')
...
- up
- see request-detail-page
- click comment-item [team.partial_crud_reply_target_comment] reply-link
- scope reply-dialog
...
- up
- see request-detail-page
- scope comment-item [team.partial_phrase_linked_seed_comment]

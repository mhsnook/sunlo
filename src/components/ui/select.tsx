...
const SelectTrigger = ({
	...
	return (
		<SelectPrimitive.Trigger
			data-slot="select-trigger"
			className={cn(
				'data-[placeholder]:text-con-mid focus:outline-con-high flex h-10 w-full items-center justify-between rounded-2xl px-3 py-2 text-sm shadow-sm focus:outline-2 focus:outline-offset-2 disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1',
				className
			)}
			{...props}
		>
...
const SelectItem = ({
	...
	<SelectPrimitive.Item
		...
		className={cn(
			'focus:hue-accent focus:bg-lum-2 focus:text-con-high focus:text-chroma-mlow focus:text-hue-accent relative flex w-full cursor-default items-center rounded-2xl py-1.5 pr-2 pl-8 text-sm outline-none select-none data-[disabled]:pointer-events-none data-[disabled]:opacity-50',
			className
		)}
		{...props}
	>

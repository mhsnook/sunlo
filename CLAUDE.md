...
- **Follow-up work after a merge goes on a NEW branch name, never the merged one.** Once a branch's PR merges, that branch is done — reusing its name re-attaches to the closed PR and clutters the branch list. Branch fresh off the default (`git checkout -B <new-name> origin/main`) and push that. The `guard-merged-branch-push.sh` pre-push hook enforces this.
...
**Mutations**: persistence lives on the collection (`onInsert/onUpdate` — a removal is a soft delete, so it reaches the collection as an update); components call `collection.insert/update` and wire toasts to `tx.isPersisted.promise`. Throwing from the handler rolls back the optimistic state. `.select()` the write and write the returned rows back with `writeSyncedRow`/`writeSyncedRows`, which lets the handler skip reloading the whole table (`{ refetch: false }`). The optimistic value drops when the transaction ends, and the user sees whatever the synced layer holds. The old `useMutation` + manual `writeInsert`-in-`onSuccess` pattern is deprecated (`transform` label). Worked examples and exceptions: `docs/mutations.md`.

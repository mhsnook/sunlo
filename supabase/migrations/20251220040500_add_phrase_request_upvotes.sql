...
create table "public"."phrase_request_upvote" (
...
create or replace function public.update_phrase_request_upvote_count () returns trigger language plpgsql security definer as $function$
begin
...
create trigger on_phrase_request_upvote_added
after insert on public.phrase_request_upvote for each row
execute function public.update_phrase_request_upvote_count ();

create trigger on_phrase_request_upvote_removed
after delete on public.phrase_request_upvote for each row
execute function public.update_phrase_request_upvote_count ();

-- Toggle function
create or replace function public.toggle_phrase_request_upvote (p_request_id uuid) returns json language plpgsql as $function$
DECLARE
...
,
	insert,
	delete on table "public"."phrase_request_upvote" to "authenticated";
...
add constraint "phrase_request_upvote_pkey" primary key using index "phrase_request_upvote_pkey";

...
--
-- WARNING: this insert runs inside gotrue's email-confirmation transaction.
-- If it ever throws, the confirmation rolls back and the signup fails. Keep
-- the column list minimal — any future NOT NULL column added to user_profile
-- without a default will break every new signup until it is also set here.
...
after update of "email_confirmed_at" on "auth"."users" for each row when (

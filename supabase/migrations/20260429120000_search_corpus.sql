...
-- parent phrase.
--
-- Throwaway-friendly: populate via scripts/backfill-search-corpus.ts. When
...
-- Btree on text_normalized to make the embedding-cache lookup O(log n)
-- instead of seq-scan. The Edge Function uses this to skip a Workers AI
-- call when the user's normalized query exactly matches some indexed
-- text_normalized — see search/index.ts for the gotchas.
create index if not exists "search_corpus_text_normalized_idx" on "public"."search_corpus" ("text_normalized");

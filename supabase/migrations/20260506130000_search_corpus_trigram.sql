...
begin
	...
	deduped as (
		select distinct on (entity_id)
			entity_id, entity_type, source_type, matched_text, matched_lang, sim_score, created_at
		from live_pool
		order by entity_id, sim_score desc
	)
	select
		deduped.entity_type,
		deduped.entity_id,
		deduped.source_type as matched_via,
		deduped.matched_text,
		deduped.matched_lang,
		deduped.sim_score::real as similarity,
		deduped.created_at
	from deduped
	where (cursor_created_at is null and cursor_id is null)
		or (deduped.created_at, deduped.entity_id) < (cursor_created_at, cursor_id)
	order by deduped.sim_score desc, deduped.created_at desc, deduped.entity_id desc
	limit match_limit;
end;
$$;

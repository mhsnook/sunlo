...
The phrases domain uses two named collections instead of an `*Active` suffix, because both collections have callers. `phrasesComposed` holds every phrase and every translation; `useLangPhrasesRaw` and `useOnePhrase` read `phrasesComposed` for the two admin screens, which must show an archived phrase so an admin can moderate the archived phrase. `phrasesFull` drops archived phrases and archived translations; browse, search, review and playlists read `phrasesFull`. `AddTranslationsDialog` is the one exception, and queries `phraseTranslationsCollection` directly, because the dialog restores an archived translation and must list the archived translation first.

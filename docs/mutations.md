...
Phrases name the pair rather than suffixing one of them, because both halves have real readers. `phrasesComposed` keeps every row and every translation, and the two admin screens read it, because moderating an archived phrase means seeing it. `phrasesFull` drops the archived phrase and any archived translation on it, and every other surface reads that one — browse, search, review, playlists. The manage-translations dialog is the deliberate exception, and it takes `useAllPhraseTranslations(pid)` rather than filtering by hand, because it restores what it shows.
